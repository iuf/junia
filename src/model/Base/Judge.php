...
/**
 * Base class that represents a row from the 'kk_junia_judge' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class Judge implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\iuf\\junia\\model\\Map\\JudgeTableMap';
    ...
    /**
     * Initializes internal state of iuf\junia\model\Base\Judge object.
     */
    ...
    /**
     * Compares this with another <code>Judge</code> instance.  If
     * <code>obj</code> is an instance of <code>Judge</code>, delegates to
     * <code>equals(Judge)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Judge The current object, for fluid interface
     */
     ...
     * @return $this|\iuf\junia\model\Judge The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[JudgeTableMap::COL_ID] = true;
        }
    ...
    public function setPosition($v)
    {
        ...
        if ($this->position !== $v) {
            $this->position = $v;
            $this->modifiedColumns[JudgeTableMap::COL_POSITION] = true;
        }
    ...
    public function setStartgroupId($v)
    {
        ...
        if ($this->startgroup_id !== $v) {
            ...
            $this->modifiedColumns[JudgeTableMap::COL_STARTGROUP_ID] = true;
        }
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[JudgeTableMap::COL_USER_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : JudgeTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : JudgeTableMap::translateFieldName('Position', TableMap::TYPE_PHPNAME, $indexType)];
            $this->position = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : JudgeTableMap::translateFieldName('StartgroupId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->startgroup_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : JudgeTableMap::translateFieldName('UserId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = JudgeTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\iuf\\junia\\model\\Judge'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildJudgeQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildJudgeQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                JudgeTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[JudgeTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . JudgeTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(JudgeTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(JudgeTableMap::COL_POSITION)) {
            $modifiedColumns[':p' . $index++]  = '`position`';
        }
        if ($this->isColumnModified(JudgeTableMap::COL_STARTGROUP_ID)) {
            $modifiedColumns[':p' . $index++]  = '`startgroup_id`';
        }
        if ($this->isColumnModified(JudgeTableMap::COL_USER_ID)) {
            $modifiedColumns[':p' . $index++]  = '`user_id`';
        }

        $sql = sprintf(
            'INSERT INTO `kk_junia_judge` (%s) VALUES (%s)',
    ...
    {
        $pos = JudgeTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\iuf\junia\model\Judge
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\iuf\junia\model\Judge The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(JudgeTableMap::DATABASE_NAME);

        if ($this->isColumnModified(JudgeTableMap::COL_ID)) {
            $criteria->add(JudgeTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(JudgeTableMap::COL_POSITION)) {
            $criteria->add(JudgeTableMap::COL_POSITION, $this->position);
        }
        if ($this->isColumnModified(JudgeTableMap::COL_STARTGROUP_ID)) {
            $criteria->add(JudgeTableMap::COL_STARTGROUP_ID, $this->startgroup_id);
        }
        if ($this->isColumnModified(JudgeTableMap::COL_USER_ID)) {
            $criteria->add(JudgeTableMap::COL_USER_ID, $this->user_id);
        }
    ...
    {
        $criteria = ChildJudgeQuery::create();
        $criteria->add(JudgeTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \iuf\junia\model\Judge (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \iuf\junia\model\Judge Clone of current object.
     * @throws PropelException
     */
    ...
    public function getStartgroup(ConnectionInterface $con = null)
    {
        if ($this->aStartgroup === null && ($this->startgroup_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aStartgroup->addJudges($this);
             */
        }
     ...
     * If this ChildJudge is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildJudge The current object (for fluent API support)
     */
     ...
     * Otherwise if this Judge is new, it will return
     * an empty collection; or if this Judge has previously
     * been saved, it will retrieve related Scores from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Judge.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildScore[] List of ChildScore objects
     */
    public function getScoresJoinRoutine(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildScoreQuery::create(null, $criteria);
        $query->joinWith('Routine', $joinBehavior);
    ...
    public function setPerformanceScores(Collection $performanceScores, ConnectionInterface $con = null)
    {
        ...
        foreach ($performanceScoresToDelete as $performanceScoreRemoved) {
            $performanceScoreRemoved->setJudge(null);
        }
    ...
    protected function doAddPerformanceScore(ChildPerformanceScore $performanceScore)
    {
        ...
        $performanceScore->setJudge($this);
    }
    ...
    public function removePerformanceScore(ChildPerformanceScore $performanceScore)
    {
        if ($this->getPerformanceScores()->contains($performanceScore)) {
            ...
            $performanceScore->setJudge(null);
        }
     ...
     * @return ObjectCollection|ChildPerformanceScore[] List of ChildPerformanceScore objects
     */
    public function getPerformanceScoresJoinRoutine(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPerformanceScoreQuery::create(null, $criteria);
        $query->joinWith('Routine', $joinBehavior);
    ...
    {
        if (null !== $this->aStartgroup) {
            $this->aStartgroup->removeJudge($this);
        }
        if (null !== $this->aUser) {
            $this->aUser->removeJudge($this);
        }
        $this->id = null;
        $this->position = null;
    ...
    {
        return (string) $this->exportTo(JudgeTableMap::DEFAULT_STRING_FORMAT);
    }
