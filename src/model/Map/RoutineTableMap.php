...
/**
 * This class defines the structure of the 'kk_junia_routine' table.
 *
 *
 *
 ...
 *
 */
class RoutineTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.RoutineTableMap';
     ...
     */
    const TABLE_NAME = 'kk_junia_routine';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\iuf\\junia\\model\\Routine';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Routine';
     ...
     */
    const COL_ID = 'kk_junia_routine.id';

    /**
     * the column name for the name field
     */
    const COL_NAME = 'kk_junia_routine.name';
    ...
    {
        // attributes
        $this->setName('kk_junia_routine');
        $this->setPhpName('Routine');
        $this->setIdentifierQuoting(true);
        $this->setClassName('\\iuf\\junia\\model\\Routine');
        $this->setPackage('');
  ...
  array (
    ...
    /**
     * Method to invalidate the instance pool of all tables related to kk_junia_routine     * by a foreign key with ON DELETE CASCADE
     */
    ...
    {
        return $withPrefix ? RoutineTableMap::CLASS_DEFAULT : RoutineTableMap::OM_CLASS;
    }
     ...
     * @return array           (Routine object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = RoutineTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = RoutineTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + RoutineTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = RoutineTableMap::OM_CLASS;
            /** @var Routine $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            RoutineTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = RoutineTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = RoutineTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(RoutineTableMap::DATABASE_NAME)->getTable(RoutineTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(RoutineTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(RoutineTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new RoutineTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Routine or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Routine object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \iuf\junia\model\Routine) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(RoutineTableMap::DATABASE_NAME);
            $criteria->add(RoutineTableMap::COL_ID, (array) $values, Criteria::IN);
        }
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                RoutineTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return RoutineQuery::create()->doDeleteAll($con);
    }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Routine object
        }
