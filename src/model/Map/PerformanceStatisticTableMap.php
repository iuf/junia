...
/**
 * This class defines the structure of the 'kk_junia_performance_statistic' table.
 *
 *
 *
 ...
 *
 */
class PerformanceStatisticTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.PerformanceStatisticTableMap';
     ...
     */
    const TABLE_NAME = 'kk_junia_performance_statistic';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\iuf\\junia\\model\\PerformanceStatistic';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'PerformanceStatistic';
     ...
     */
    const COL_ID = 'kk_junia_performance_statistic.id';

    /**
     * the column name for the min field
     */
    const COL_MIN = 'kk_junia_performance_statistic.min';

    /**
     * the column name for the max field
     */
    const COL_MAX = 'kk_junia_performance_statistic.max';
    ...
    {
        // attributes
        $this->setName('kk_junia_performance_statistic');
        $this->setPhpName('PerformanceStatistic');
        $this->setIdentifierQuoting(true);
        $this->setClassName('\\iuf\\junia\\model\\PerformanceStatistic');
        $this->setPackage('');
  ...
  array (
    ...
    {
        return $withPrefix ? PerformanceStatisticTableMap::CLASS_DEFAULT : PerformanceStatisticTableMap::OM_CLASS;
    }
     ...
     * @return array           (PerformanceStatistic object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PerformanceStatisticTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PerformanceStatisticTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PerformanceStatisticTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PerformanceStatisticTableMap::OM_CLASS;
            /** @var PerformanceStatistic $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PerformanceStatisticTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PerformanceStatisticTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PerformanceStatisticTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PerformanceStatisticTableMap::DATABASE_NAME)->getTable(PerformanceStatisticTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PerformanceStatisticTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PerformanceStatisticTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PerformanceStatisticTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a PerformanceStatistic or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or PerformanceStatistic object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \iuf\junia\model\PerformanceStatistic) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(PerformanceStatisticTableMap::DATABASE_NAME);
            $criteria->add(PerformanceStatisticTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = PerformanceStatisticQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            PerformanceStatisticTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                PerformanceStatisticTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PerformanceStatisticQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a PerformanceStatistic or Criteria object.
     *
     * @param mixed               $criteria Criteria or PerformanceStatistic object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from PerformanceStatistic object
        }

        if ($criteria->containsKey(PerformanceStatisticTableMap::COL_ID) && $criteria->keyContainsValue(PerformanceStatisticTableMap::COL_ID) ) {
...
//
PerformanceStatisticTableMap::buildTableMap();
